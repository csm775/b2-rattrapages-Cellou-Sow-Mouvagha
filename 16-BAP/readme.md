# 16 - Rattrapage BAP - ESM Escalade

## Contexte
Site e-commerce WordPress pour le club ESM Escalade (Massy), permettant aux adhérents
de customiser et acheter des chaussons d'escalade en souvenir des Jeux Olympiques de Paris.

## Stack technique
- WordPress (CMS)
- WooCommerce (gestion e-commerce)
- Thème : [à compléter]
- Plugin(s) de customisation produit : [à compléter]
- Plugin(s) de paiement : [à compléter]

## Cahier des charges
- [ ] Site responsive avec UI soignée
- [ ] Page d'accueil (landing page) avec logo ESM Escalade, mise en avant des JO et des
      champions Zélia et Sam Avezou
- [ ] Page de résultats de recherche
- [ ] Page produits avec customisation (visuels JO Paris) + attributs marque/taille/rigidité
- [ ] Page de paiement sécurisée

## Setup / Installation
[À compléter : comment lancer le site en local avec Local (LocalWP), ou instructions
si export WordPress fourni]

## Recherches effectuées
[Lister ici toute recherche internet faite pendant l'exercice, même non concluante,
avec les liens]

## Vidéo explicative
Lien : [à compléter - Loom ou YouTube non répertorié]

La vidéo présente :
- Démonstration du site
- Explication technique de la configuration WordPress/WooCommerce
- Choix réalisés (thème, plugins, structure des produits)